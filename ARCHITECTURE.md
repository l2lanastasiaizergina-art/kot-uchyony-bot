...
├── english_vocabulary.py  # уровни, тесты и интервальное повторение английского
├── database/              # миграции, модели, репозитории
└── utils/                 # нормализация текста и время
data/words/                # пакеты слов 1–11 классов
data/english_vocabulary/   # 480 слов и 1 440 тестовых заданий
...
Wise Cat English использует отдельные сессии и ответы. Ключи не передаются в
браузер до фиксации попытки. Ошибка создаёт карточку повторения; успешные
повторения переводят её по интервалам 1 → 3 → 7 → 14 дней, после чего карточка
считается закреплённой.
